...
public abstract class AbstractResource<T> {
    ...
    static final String ENTITY_PATH = "/{Id: [0-9]*}";
     ...
     * @return whether the item was created
     */
    @POST
    ...
    public T update(@ApiParam(value = "Entity object that needs to be updated in the database", required = true) T t,
                    @PathParam(ID_NAME) @ApiParam(value = "ID of entity that needs to be updated", required = true) Optional<Integer> id) {
        if (id.isPresent()) {
    ...
    public boolean delete(
            @ApiParam(value = "ID of entity that needs to be deleted", required = true)
            @PathParam(ID_NAME)
                    Optional<Integer> id) {
        if (id.isPresent()) {
